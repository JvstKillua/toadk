...
function animateteksnim() {
    ...
    setTimeout(function() {
        const intervalId = setInterval(() => {
            if (percent < 17) {
                percent += Math.floor(Math.random() * (1 - 10 + 1)) + 10;
                const randomEmoji = loveEmojis[Math.floor(Math.random() * loveEmojis.length)];
                teksLove.innerHTML = `<b>${defTeksLove} ${percent} tahun ${randomEmoji}</b>`;
            } else {
                ...
                percent = 17;
                const randomEmoji = loveEmojis[Math.floor(Math.random() * loveEmojis.length)];
                teksLove.innerHTML = `<b>${defTeksLove} <span style='color:yellow'>${percent} tahun</span> ${randomEmoji}</b>`;
...
function katanimasi3(){
    ...
    new TypeIt("#teks3", {
        strings: Array.from({length: 10}, (_, i) => `HBD!!! ${i + 1}% ${ambilRandomEmoji}`),
        startDelay: 1,
        speed: 0.2,
        cursor: false,
        nextStringDelay: 0,
